...
<div id="content" style="font-size:150%;"><div style="font-size:200%";><b>Happening events in <?php echo $_SESSION["city"];?></b></div>
<?php
...
$result = mysqli_query($con,"SELECT events FROM Events WHERE place='$a'");
...
$count=mysqli_query($con,"SELECT count(id),min(id) FROM Events"); 
	if($row=mysqli_fetch_array($count)) {
		if($row['count(id)']==3) {
			$sql="DELETE from Events WHERE id=".$row['min(id)'];
			$result=mysqli_query($con,$sql);
		}
	}
	$output=str_replace("\"","\\\"",$output);
	$sql="INSERT INTO Events(place,events) VALUES(\"$a\",\"$output\")";
